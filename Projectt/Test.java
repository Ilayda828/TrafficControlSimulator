...
public class Test extends Application {
	  ...
	  private Button next;
	  private File[] files=new File[] {new File("/Users/ilayda/Desktop/level1.txt"),
		  new File("/Users/ilayda/Desktop/level2.txt"),
		  new File("/Users/ilayda/Desktop/level3.txt"),
		  new File("/Users/ilayda/Desktop/level4.txt"),
		  new File("/Users/ilayda/Desktop/level5.txt")};
	  
	public void start(Stage s) throws Exception {
		
		this.s = s;
		final File[] initialFile = new File[1];
		Image im = new Image("file:/Users/ilayda/Desktop/image.jpg");
			...
			if (ob instanceof Metadata) {
				...
				for (int row = 0; row < cellsOfY; row++) {
					for (int col = 0; col < cellsOfX; col++) {
						Rectangle r = new Rectangle(col*((width / cellsOfX)), row*(height / cellsOfY),(width / cellsOfX), (height / cellsOfY));
				...
				for (Object ob1 : a) {
					if (ob1 instanceof Building) {
						...
						if (type == 0) {
							...
							Rectangle rIn = new Rectangle(9,9,((width / cellsOfX) * 2) - 18, ((height / cellsOfY) * 2) - 18);
							Rectangle rIn2 = new Rectangle(17,17,((width / cellsOfX) * 2) - 34, ((height / cellsOfY) * 2) - 34);
						...
						if (type == 2) {
							...
							Rectangle r = new Rectangle((width / cellsOfX)*cellX,(height / cellsOfY)*cellY,(width / cellsOfX), (height / cellsOfY));
				...
				for (Object ob2 : a) {
					if (ob2 instanceof RoadTile) {
						...
						if (type == 0) {
							Rectangle r = new Rectangle((width / cellsOfX)*cellX,(height / cellsOfY)*cellY+5,(width / cellsOfX), (height / cellsOfY) - 10);
						...
						if (type == 1) {
							Arc arc = new Arc( 0.1, (height / cellsOfY), (width / cellsOfX) - 5+0.1, (height / cellsOfY) - 5, 0, 90);
						...
						if (type == 2) {
							...
							Rectangle r = new Rectangle( cellX*(width / cellsOfX), cellY*(height / cellsOfY)+5,(width / cellsOfX), (height / cellsOfY) - 10);
							r.setFill(Color.WHITE);
                            
							Rectangle r1 = new Rectangle( cellX*(width / cellsOfX)+5, cellY*(height / cellsOfY),(width / cellsOfX) - 10, (height / cellsOfY));
	...
	/*	String musicFile = new String("C://Users//USER//OneDrive//Masaüstü//carGameVoice.mp3");
		...
		this.timer = new AnimationTimer() {
	        ...
	        public void handle(long now) {
				...
				update(file,gp,score,crashes,stop,back); //,mediaPlayer);
	...
	private void update(File file,Pane gp,Label score,Label crashes,Button stop,Button back) { //,MediaPlayer mediaPlayer) {
		...
		if(time > 2) {
			if(Math.random()<0.6) {
				...
				if(this.score<100 && this.crashes>=10) {
		    		...
		    		popupLayout.setPrefSize(400, 400);
		    		popupLayout.setAlignment(Pos.CENTER);
				    Label messageLabel = new Label("Game is Over!!!");
				    messageLabel.setFont(new Font("Arial",30));
				...
				spawnCar(gp,score,crashes,stop,back); //,mediaPlayer);
				}
	...
	private void spawnCar(Pane gp,Label score,Label crashes,Button stop,Button back) { //,MediaPlayer mediaPlayer) {
	...
	public void checkLights() { 
		...
		for (int i = 0; i < carTransitions.size(); i++) {
		    for (Circle light : lights) {
		        Bounds carBounds = carTransitions.get(i).getNode().localToScene(carTransitions.get(i).getNode().getBoundsInLocal());
